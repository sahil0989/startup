...
export default function Categories() {
    ...
    useEffect(() => {
        if (collections && collections.length > 0) {
            // Shuffle and pick 4 unique random categories
            const randomFour = [...collections]
                .sort(() => Math.random() - 0.5)
                .slice(0, 4);

            setHomeCollection(randomFour);
        }
    }, [collections]);
    ...
    return (
        <div className="bg-gray-50 py-16">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-10 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
                    {homeCollection?.map((category, index) => (
